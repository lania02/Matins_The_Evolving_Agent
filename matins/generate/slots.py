...
def _idea_schema_instruction() -> str:
    ...
    return (
        ...
        "GROUNDING (the single most important rule): the idea's TARGET must be a REAL, "
        "nameable thing that exists in the world -- an observable phenomenon, an accessible "
        "dataset, a decision a real person makes, or a running system -- NOT a second piece "
        "of mathematics. Mapping one formalism onto another formalism (math x math) with a "
        "real application bolted on afterward WILL BE REJECTED. The math is the tool; the "
        "target must be something a normal person can point at. "
        "Write \"intuition\" FIRST and in PLAIN language a smart non-specialist grasps in ~10 "
        "seconds: NO jargon, NO symbols -- name the concrete real situation this is about and "
        "why anyone would care. If you cannot explain it without formalism, the target is too "
        "abstract; pick a more grounded one. "
        "Make \"tractability\" a RUNNABLE weekend artifact: exactly what you would build and "
        "RUN, the specific named/accessible input data, what it computes, and the tangible "
        "output you would SEE (a plot, a number, a ranked list) -- never 'this could be "
        "implemented'. "
        ...
        "two coincide.\" "
        "Write \"elaboration\" as the idea's walkthrough, in FOUR tight parts, ONE sentence "
        "EACH (four sentences total, ~350-500 characters combined -- concrete and dense, not "
        "padded prose; a wall of text is as much a failure as a one-liner): (1) the "
        "CONSTRUCTION -- the objects, with an actual quantity or a tiny worked instance, not "
        "just their names; (2) the LOAD-BEARING ARGUMENT -- the one step a skeptic would "
        "attack, stated and defended, NOT a restatement of the title; (3) the KEY ASSUMPTION "
        "and the regime where it breaks; (4) the FIRST EXPERIMENT -- what exactly you "
